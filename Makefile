...
.PHONY: sync-agents check-agents link smoke-cli test check-layout check-debt scan-deps generate-sbom install-pre-commit run-pre-commit check-pre-commit-config ci-bootstrap-ubuntu format-cpp check-clang-format check-static-analysis check-cmake-starter ci-checks
...
check-debt:
	$(PYTHON) tools/cli.py check-debt

scan-deps:
	$(PYTHON) tools/dev/scan_dependencies.py

generate-sbom:
	$(PYTHON) tools/dev/generate_sbom.py
...
ci-checks: check-agents smoke-cli test check-layout check-debt check-pre-commit-config check-clang-format check-static-analysis check-cmake-starter
