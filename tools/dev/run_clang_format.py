...
def filter_paths(argv_paths: list[str]) -> list[Path]:
    ...
    for raw_path in argv_paths:
        ...
        if file_path.suffix not in SOURCE_SUFFIXES:
            continue
        if STARTER_DIR not in file_path.parents:
            continue
...
def main() -> int:
    ...
    file_paths = filter_paths(sys.argv[1:]) if len(sys.argv) > 1 else iter_starter_source_files()
    if not file_paths:
        return 0

    subprocess.run(
        ["clang-format", "-i", "--style=file", *(str(path) for path in file_paths)],
        cwd=REPO_ROOT,
        check=True,
    )
    return 0
